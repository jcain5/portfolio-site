...
import { jobs, type Job } from "../lib/experience";
...
export default function Experience() {
  const professionalJobs = jobs.filter((j) => j.source === "professional");
  const volunteerJobs = jobs.filter((j) => j.source === "volunteer");
  ...
  return (
    <Section
      ...
      intro={
        <>
          Enterprise IT operations experience, reinforced by hands-on infrastructure work through my{" "}
          <Link href="/projects/enterprise-infrastructure-lab" className="text-[#2F75C8] hover:underline">
            Enterprise Infrastructure Lab
          </Link>
          , where I design, build, and administer Windows Server, identity, virtualization, and networking systems
          independently — distinct from the enterprise environments I support professionally.
        </>
      }
      tone="alt"
      maxWidth="4xl"
    ...
    >
      <div className="space-y-14">
        <div>
          <h2 className="font-heading text-xl font-semibold text-ink mb-6">Professional Experience</h2>
          <Timeline jobs={professionalJobs} />
        </div>

        {volunteerJobs.length > 0 && (
          <div>
            <h2 className="font-heading text-xl font-semibold text-ink mb-6">Volunteer Experience</h2>
            <Timeline jobs={volunteerJobs} />
          </div>
        )}
      </div>

      <div className="mt-8 pl-0 sm:pl-12">
        <p className="text-xs font-mono text-muted uppercase tracking-wide mb-2">Related Independent Project</p>
        <Link
